...
# Function
qp <- function(x = '', y = '', geo = '') {
    p <- qplot(x, y, data = df_nat_toplines, geom = geo)
    p
}

# National Toplines
df_nat_toplines <- read_csv('presidential_national_toplines_2020_nov2.csv')
df_nat_toplines <- subset(df_nat_toplines, select = -c(cycle, branch, model, candidate_inc, candidate_chal, candidate_3rd, popwin_3rd, ev_3rd, ev_3rd_hi, ev_3rd_lo, national_voteshare_3rd_hi, national_voteshare_3rd_lo, ecwin_3rd, national_voteshare_3rd, simulations))
...
p_date_ev_inc <- p_date_ev_inc + ylim(190, 350)
